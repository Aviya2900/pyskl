...
@DATASETS.register_module()
class PoseDataset_2P(BaseDataset):
    """Pose dataset for action recognition.
    ...
    def load_pkl_annotations(self):
        ...
        data = [self.label_mapper(d) for d in data]

        for item in data:
            # Sometimes we may need to load anno from the file
            if 'filename' in item: item['filename'] = osp.join(self.data_prefix, item['filename'])
            if 'frame_dir' in item: item['frame_dir'] = osp.join(self.data_prefix, item['frame_dir'])
        return data
    
    def label_mapper(self, data):
        label_map = {50: 0, 51: 1, 52: 2, 53: 3, 54: 4, 55: 5, 56: 6, 57: 7, 58: 8, 59: 9, 60: 10,
                    106: 11, 107: 12, 108: 13, 109: 14, 110: 15, 111: 16, 112: 17, 113: 18, 114: 19,
                    115: 20, 116: 21, 117: 22, 118: 23, 119: 24, 120: 25}

        # Update the label in the data dictionary
        label = data['label']
        data['label'] = label_map.get(label + 1, -1)  # Adjust label with +1 and map it
        return data
