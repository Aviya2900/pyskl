...
@RECOGNIZERS.register_module()
class MMRecognizer3D_SAP(BaseRecognizer):
    """MultiModality 3D recognizer model framework for SAP representation"""

    def forward_train(self, feats, heatmap_imgs, label, img_metas=None):
        """Defines the computation performed at every call when training."""
        labels = label
        assert not hasattr(self, 'neck')
        # It's OK, 1 clip training
        ...
        # We directly use backbone instead of extract_feats
        # x = self.extract_feat(imgs)
        x_feats, x_pose = self.backbone(feats, heatmap_imgs)

        # Which will return 3 cls_scores: ['feats', 'pose', 'both']
        cls_scores = self.cls_head((x_feats, x_pose))
    ...
    def forward_test(self, feats, heatmap_imgs, img_metas=None):
        """Defines the computation performed at every call when evaluation and
        testing."""
        assert feats.shape[0] == 1 and heatmap_imgs.shape[0] == 1
        assert not hasattr(self, 'neck')
        feats = feats.reshape((-1, ) + feats.shape[2:])
        heatmap_imgs = heatmap_imgs.reshape((-1, ) + heatmap_imgs.shape[2:])
    ...
    def forward(self, feats, heatmap_imgs, label=None, return_loss=True, **kwargs):
        """Define the computation performed at every call."""
        if return_loss:
            if label is None:
                raise ValueError('Label should not be None.')
            return self.forward_train(feats, heatmap_imgs, label, **kwargs)

        return self.forward_test(feats, heatmap_imgs, **kwargs)
