...
class MMPoseHead(BaseHead):
    ...
    def __init__(self,
        ...
        super().__init__(num_classes, in_channels, loss_cls, **kwargs)
        if isinstance(dropout, float):
            dropout = {'feats': dropout, 'pose': dropout}
        assert isinstance(dropout, dict)
        ...
        self.dropout_feats = nn.Dropout(p=self.dropout['feats'])
        self.dropout_pose = nn.Dropout(p=self.dropout['pose'])

        self.fc_feats = nn.Linear(in_channels[0], num_classes)
        self.fc_pose = nn.Linear(in_channels[1], num_classes)
        self.avg_pool = nn.AdaptiveAvgPool3d((1, 1, 1))

    def init_weights(self):
        """Initiate the parameters from scratch."""
        normal_init(self.fc_feats, std=self.init_std)
        normal_init(self.fc_pose, std=self.init_std)
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: The classification scores for input samples.
        """
        x_feats, x_pose = self.avg_pool(x[0]), self.avg_pool(x[1])
        x_feats = x_feats.view(x_feats.size(0), -1)
        x_pose = x_pose.view(x_pose.size(0), -1)

        x_feats = self.dropout_feats(x_feats)
        x_pose = self.dropout_pose(x_pose)

        cls_scores = {}
        cls_scores['feats'] = self.fc_feats(x_feats)
        cls_scores['pose'] = self.fc_pose(x_pose)
